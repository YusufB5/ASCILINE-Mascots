class SpriteMascot extends Mascot {
    ...
    renderHitboxOverlay() {
        ...
        // Measure grid cell dimensions based on metadata or frame length
        ...
        if (this.frames && this.frames.length > 0) {
            if (typeof this.frames[0] === 'string') {
                ...
                cols = (lines[0] && lines[0].length) || cols;
        ...
        this.customPoints.forEach(pt => {
            ...
            if (pt.type === 'rect') {
                ...
                svgHtml += `<circle cx="${cx}" cy="${cy}" r="3" fill="#ff00cc" stroke="#ffffff" stroke-width="1" />`;
            }
        });
