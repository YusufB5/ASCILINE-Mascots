...
class SwimmerMascot extends SpriteMascot {
    ...
    freeFloatMovement(dt = 1) {
        ...
        switch (this.swimState) {
            case 'SWIM_RIGHT':
                this.walkDir = 1;
                this.vx = this.walkSpeed;
                this.pre.style.transform = `scaleX(${this.facingRight ? 1 : -1})`; 
    ...
    landPhysicsMovement(water, dt = 1) {
        ...
        // Apply correct sprite orientation based on walkDir
        const dirScale = this.walkDir === 1 ? (this.facingRight ? 1 : -1) : (this.facingRight ? -1 : 1);
        this.pre.style.transform = `scaleX(${dirScale})`;

        // Floor boundary
        const floorY = window.scrollY + window.innerHeight - this.height - 20;
        if (nextY >= floorY) {
            nextY = floorY;
            if (this.vy > 1.5) {
