...
class JumperPhysics extends SpriteMascot {
  constructor(jsonUrl, width = 80, height = 80, fps = 15, x = null, y = null, customConfig = null) {
    super(jsonUrl, width, height, fps, 0, null);
    ...
    // Random color tint for slime (only for default slime URL, not raw objects or other mascots)
    if (typeof jsonUrl === 'string' && jsonUrl.includes('slime')) {
        const colors = [
            null,                          // original green
  ...
  tick(dt = 1) {
    ...
    // Apply scale transforms on wrapper (combining squish and flip)
    const needsFlip = (this.facing === 'left' && this.walkDir === 1) || 
                      (this.facing === 'right' && this.walkDir === -1);
    const flipScale = needsFlip ? -1 : 1;

    if (this.allowSquish) {
      this.wrapper.style.transformOrigin = "50% 100%";
      this.wrapper.style.transform = `translate3d(${this.x}px, ${this.y}px, 0) scale(${this.squishAmount * flipScale}, ${this.stretchAmount})`;
    } else {
      this.updateDOMPosition();
    }
...
class BouncerMascot extends SpriteMascot {
    constructor(jsonUrl, width = 90, height = 90, fps = 15) {
        super(jsonUrl, width, height, fps);
    ...
    tick(dt = 1) {
        ...
        // Rolling effect — rotate the wrapper so the collision box rotates with the visual
        this.rot += this.vx * 2.5 * dt;
        this.renderFrame(this.animFrame);

        // Apply position + rotation on wrapper together
        const needsFlip = (this.facing === 'left' && (this.walkDir || 1) === 1) ||
                          (this.facing === 'right' && (this.walkDir || 1) === -1);
        const flip = needsFlip ? ' scaleX(-1)' : '';
...
class BombPhysics extends SpriteMascot {
    constructor(xOrData = 0, y = 0, jsonUrlOrData = null) {
        let initialX = 0;
        let initialY = 0;
        let mascotData = jsonUrlOrData || 'assets/mascots/bom3_coloranim.json';

        if (typeof xOrData === 'string' || (xOrData && typeof xOrData === 'object' && !xOrData.nodeType)) {
            mascotData = xOrData;
            initialX = y || 0;
            initialY = arguments[2] || 0;
        } else {
            initialX = xOrData;
            initialY = y;
        }

        super(mascotData, 120, 120, 30, 0, null); // walkspeed = 0
        this.x = initialX;
        this.y = initialY;
                ...
                if (spans && spans.length > 0) {
                    ...
                    rowsMap.forEach((rowItems) => {
                        ...
                        for (let i = 0; i < rowItems.length; i += chunkSize) {
                            ...
                            if (typeof DomPhysicsObject !== 'undefined') {
                                const shard = new DomPhysicsObject(
